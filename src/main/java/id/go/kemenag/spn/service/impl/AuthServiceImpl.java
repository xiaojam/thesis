...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        System.out.println(AuthUtil.hash(request.getPassword()));
    ...
    public LoginResponse refreshToken(RefreshTokenRequest request) {
        ...
        if (!this.jwtService.isTokenValid(refreshToken, user)) {
            ErrorUtil.throwError("Invalid or expired refresh token", HttpStatus.UNAUTHORIZED);
        }

        String savedToken = this.redisService.get("refresh:" + username);
        if (savedToken == null || !savedToken.equals(refreshToken)) {
            ErrorUtil.throwError("Refresh token not recognized", HttpStatus.UNAUTHORIZED);
        }
