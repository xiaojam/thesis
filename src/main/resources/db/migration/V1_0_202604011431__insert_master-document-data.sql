...
SELECT
    m.id,
    'Sirojjam Mushlich, M.Th.I.',
    'B.{{SEQ_3}}/Kua.{{KODE_KEC}}/PW.01/{{ROMAN_MONTH}}/{{YYYY}}',
    0,
    'MARRIAGE',
    '-'
FROM master m
WHERE m.code = '3507091' AND m.group_name = 'KUA'
LIMIT 1;

INSERT INTO document_config (workplace_id, head_name, numbering_format, last_sequence, service_type, description)
SELECT
    m.id,
    'Robby Putra Darmadi, M.Hum.',
    '484.2/{{SEQ_3}}/DCK/{{ROMAN_MONTH}}/{{YYYY}}',
    0,
    'MARRIAGE',
    '-'
FROM master m
WHERE m.code = '35.07.09.2001' AND m.group_name = 'WILAYAH' AND m.description = 'Desa'
LIMIT 1;
...
SELECT
    ...
    '-'
FROM master m
WHERE m.code = '401427' AND m.group_name = 'PENGADILAN_AGAMA'
LIMIT 1;
...
WHERE m.code = '35.07.09.2001' AND dc.service_type = 'MARRIAGE'
LIMIT 1;
...
FROM document_config dc
JOIN master m ON dc.workplace_id = m.id
WHERE m.code = '401427' AND dc.service_type = 'DIVORCE'
LIMIT 1;
